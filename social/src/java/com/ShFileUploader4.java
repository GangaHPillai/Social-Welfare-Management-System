...
@WebServlet(name = "ShFileUploader4", urlPatterns = {"/ShFileUploader4"})
@MultipartConfig(maxFileSize = 16177215)    // upload file's size up to 16MB
public class ShFileUploader4 extends HttpServlet {
    ...
    String cat=request.getParameter("radio");
...
//String news=request.getParameter("news");  
            ...
            for (Part file : files) 
            {
                ...
                if (fname != null)
                {
                    ...
                    sql = "select ifnull(max(log_id),0)+1 from user_reg";
    ...
    sql="insert into user_reg values('"+f_id+"','"+name+"','"+cat+"','"+mob+"','"+email+"','"+add+"','"+fname+"','')";
    db.setDatas(sql, null);
    sql="insert into login values('"+email+"','"+pwd+"','user','0')";
    db.setDatas(sql,null);
